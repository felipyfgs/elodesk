...
description: Use when writing Go code that returns, wraps or maps errors — fmt.Errorf with %w (never %v), package-level sentinel vars (ErrNotFound, ErrUnsupported), handler → HTTP status mapping via errors.Is switch, parseAndValidate helpers. Triggers on any *.go file that returns error.
license: MIT
---
