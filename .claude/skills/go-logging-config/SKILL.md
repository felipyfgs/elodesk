...
description: Use when writing zerolog calls (every log MUST have .Str("component", "...") and .Err(err) for errors) or editing backend/internal/config — env var loading, required-field validation, mustDuration/getEnvAsInt helpers, fail-fast on missing JWT_SECRET / BACKEND_KEK.
license: MIT
---
