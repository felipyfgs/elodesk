...
const stats = computed<Stat[]>(() => {
  ...
  return [
    { title: t('home.stats.sessions'), icon: 'i-lucide-webhook', value: inboxes.list.length, to: `/accounts/${aid}/sessions` },
    { title: t('home.stats.connected'), icon: 'i-lucide-plug', value: connected, to: `/accounts/${aid}/sessions` },
    { title: t('home.stats.conversations'), icon: 'i-lucide-inbox', value: convs.list.length, to: `/accounts/${aid}/conversations` },
    { title: t('home.stats.unread'), icon: 'i-lucide-bell', value: unread, to: `/accounts/${aid}/conversations` }
  ]
})
      ...
      <UCard class="mt-6" :ui="{ header: 'flex items-center justify-between' }">
        ...
        <ul v-else class="flex flex-col divide-y divide-default">
          <li v-for="c in convs.list.slice(0, 8)" :key="c.id">
            <NuxtLink
              :to="`/accounts/${aid}/conversations/${c.id}`"
