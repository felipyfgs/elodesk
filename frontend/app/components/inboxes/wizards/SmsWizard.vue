...
async function validateStep(step: number): Promise<boolean> {
  if (step === 0) {
    const { error } = await smsStepSetupSchema.safeParseAsync({
      name: state.name, provider: state.provider, phoneNumber: state.phoneNumber
    })
    if (error) {
  ...
  if (step === 1) {
    const { error } = await smsStepProviderSchema.safeParseAsync({
      bandwidth: state.bandwidth, zenvia: state.zenvia
    })
    if (error) {
      providerFormRef.value?.setErrors(error.issues.map(i => ({ message: i.message, path: i.path.join('.') })))
      return false
    }
    return true
  }
...
<template>
  <InboxesWizardsBaseWizard
    :title="t('inboxes.channels.sms')"
    :description="t('inboxes.wizards.sms.description')"
    :steps="stepperItems"
