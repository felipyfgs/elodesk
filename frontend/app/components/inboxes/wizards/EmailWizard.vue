...
const state = reactive<EmailInboxForm>({
  ...
  provider: 'generic',
  email: '',
  imapAddress: '', imapPort: 993, imapLogin: '', imapPassword: '', imapEnableSsl: true, imapEnabled: true,
...
async function validateStep(step: number): Promise<boolean> {
  if (step === 0) {
    const { error } = await emailStepSetupSchema.safeParseAsync({
      name: state.name, provider: state.provider, email: state.email
    })
    if (error) {
  ...
  if (step === 1) {
    const { error } = await emailStepImapSchema.safeParseAsync({
      imapAddress: state.imapAddress, imapPort: state.imapPort, imapLogin: state.imapLogin,
      imapPassword: state.imapPassword, imapEnableSsl: state.imapEnableSsl
    })
    if (error) {
      imapFormRef.value?.setErrors(error.issues.map(i => ({ message: i.message, path: i.path.join('.') })))
      return false
    }
    return true
  }
  if (step === 2) {
    const { error } = await emailStepSmtpSchema.safeParseAsync({
      smtpAddress: state.smtpAddress, smtpPort: state.smtpPort, smtpLogin: state.smtpLogin,
      smtpPassword: state.smtpPassword, smtpEnableSsl: state.smtpEnableSsl
    })
    if (error) {
      smtpFormRef.value?.setErrors(error.issues.map(i => ({ message: i.message, path: i.path.join('.') })))
      return false
    }
    return true
  }
...
<template>
  <InboxesWizardsBaseWizard
    :title="t('inboxes.channels.email')"
    :description="t('inboxes.wizards.email.description')"
    :steps="stepperItems"
