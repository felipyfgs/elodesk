...
const state = reactive<TelegramInboxForm>({ name: '', botToken: '' })
...
async function validateStep(step: number): Promise<boolean> {
  if (step === 0) {
    const { error } = await telegramStepSetupSchema.safeParseAsync({ name: state.name })
    if (error) {
  ...
  if (step === 1) {
    const { error } = await telegramStepCredentialsSchema.safeParseAsync({ botToken: state.botToken })
    if (error) {
...
<template>
  <InboxesWizardsBaseWizard
    :title="t('inboxes.channels.telegram')"
    :description="t('inboxes.wizards.telegram.description')"
    :steps="stepperItems"
