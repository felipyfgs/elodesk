...
defineShortcuts({
  arrowdown: () => {
    ...
    if (idx === -1) next = props.items[0] ?? null
    else if (idx < props.items.length - 1) next = props.items[idx + 1] ?? null

    if (next) {
      messages.prefetch(next.id)
      selected.value = next
    }
  },
  arrowup: () => {
    ...
    if (idx === -1) prev = props.items.at(-1) ?? null
    else if (idx > 0) prev = props.items[idx - 1] ?? null

    if (prev) {
      messages.prefetch(prev.id)
      selected.value = prev
    }
  }
})
    ...
    >
      <UContextMenu :items="rowMenuItems(c)">
        <div
          ...
          @keydown.enter.prevent="prefetchMessages(c); selectConversation(c)"
          @keydown.space.prevent="prefetchMessages(c); selectConversation(c)"
