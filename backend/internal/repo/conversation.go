...
func (r *ConversationRepo) ListHydratedByIDs(ctx context.Context, accountID int64, ids []int64) (map[int64]*ConversationHydrated, error) {
	out := make(map[int64]*ConversationHydrated)
	if len(ids) == 0 {
		return out, nil
	}
	query := `SELECT ` + conversationHydratedColumns + conversationHydratedFrom +
		` WHERE cv.account_id = $1 AND cv.id = ANY($2)`
	rows, err := r.pool.Query(ctx, query, accountID, ids)
	if err != nil {
		return nil, fmt.Errorf("failed to list hydrated conversations: %w", err)
	}
	defer rows.Close()
	for rows.Next() {
		var h ConversationHydrated
		if err := scanConversationHydrated(rows, &h); err != nil {
			return nil, fmt.Errorf("failed to scan hydrated conversation: %w", err)
		}
		out[h.Conversation.ID] = &h
	}
	return out, rows.Err()
}
